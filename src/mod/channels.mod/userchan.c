...
static int mask_is_cidr(const char *mask)
{
  ...
  return slash != nullptr && str_isdigit(slash + 1);
}
                        ...
                        const char *mask, maskrec *rec)
{
  ...
  if (ss.ss_family == AF_INET)
    op_cidr_set4(*trie_ptr,
                 &((struct sockaddr_in *)&ss)->sin_addr, plen, rec, nullptr);
  else
    op_cidr_set6(*trie_ptr,
                 &((struct sockaddr_in6 *)&ss)->sin6_addr, plen, rec, nullptr);
}
...
/* Look up a host string in a trie.  Returns the matching maskrec * or nullptr. */
...
/* Destroy a trie and set the pointer to nullptr. */
static void ip_trie_destroy(op_cidr_tbl_t **trie_ptr)
{
  if (*trie_ptr) {
    op_cidr_destroy(*trie_ptr, nullptr, nullptr);
    *trie_ptr = nullptr;
  }
}
...
{
  struct chanuserrec *ch = nullptr;

  if (findchan_by_dname(chname)) {
    ch = alloc_chanuserrec();
...
{
  struct chanuserrec *ch = u->chanrec, *lst = nullptr;
...
{
  if (ht) {
    maskrec *m = op_htab_get(ht, uhost);
    return m ? (m->flags & MASKREC_STICKY) : 0;
  }
  for (; u; u = u->next)
 ...
 *   2       perm ban
 */
static int u_equals_mask(maskrec *u, op_htab *ht, char *mask)
{
  if (ht) {
    maskrec *m = op_htab_get(ht, mask);
    if (!m)
      return 0;
    return (m->flags & MASKREC_PERM) ? 2 : 1;
  }
  for (; u; u = u->next)
...
static int u_match_mask(maskrec *rec, const char *mask)
{
    ...
    if (!noshare) {
      ...
      if (mask) {
        ...
        if (c)
          ...
          shareout(nullptr, "-b %s\n", mask);
        op_free(mask);
      }
    }
    /* Remove from hash table and CIDR trie before freeing the node */
    {
      op_htab *ht = c ? c->bans_ht : global_bans_ht;
      if (ht)
        op_htab_del(ht, (*u)->mask);
    }
    if (c)
      ip_trie_del(c->ban_ip_trie, (*u)->mask);
    if (lastdeletedmask)
      op_free(lastdeletedmask);
    lastdeletedmask = op_strdup((*u)->mask);
    op_free((*u)->mask);
    if ((*u)->desc)
      op_free((*u)->desc);
    if ((*u)->user)
    ...
    if (!noshare) {
      ...
      if (mask) {
        ...
        if (c)
          ...
          shareout(nullptr, "-e %s\n", mask);
        op_free(mask);
      }
    }
    {
      op_htab *ht = c ? c->exempts_ht : global_exempts_ht;
      if (ht)
        op_htab_del(ht, (*u)->mask);
    }
    if (c)
    ...
    if (!noshare) {
      ...
      if (mask) {
        ...
        if (c)
          ...
          shareout(nullptr, "-inv %s\n", mask);
        op_free(mask);
      }
    }
    {
      op_htab *ht = c ? c->invites_ht : global_invites_ht;
      if (ht)
        op_htab_del(ht, (*u)->mask);
    }
    if (c)
...
/* Add a !* and/or @* if lacking in a usermask, writing to sb */
static void fix_broken_mask(op_strbuf_t *sb, const char *oldmask)
{
  const char *strat, *strbang;

  if (oldmask) {
    /* must have '!' and '@' */
    strat = strchr(oldmask, '@');
    strbang = strchr(oldmask, '!');
    if (strbang == nullptr && strat == nullptr)
      op_strbuf_appendf(sb, "%s!*@*", oldmask);
    else if (strat == nullptr)
      op_strbuf_appendf(sb, "%s@*", oldmask);
    else if (strbang == nullptr) {
                    ...
                    time_t expire_time, int flags)
{
  ...
  if ((me = module_find("server", 0, 0)) && me->funcs) {
    ...
    bool matched = match_addr(op_strbuf_str(&_host), op_strbuf_str(&s));
    op_strbuf_free(&s);
    if (matched) {
  ...
  p->lastactive = 0;
  p->flags = flags;
  p->mask = op_strbuf_steal(&_host);
  p->user = op_strdup(from);
  p->desc = op_strdup(note);
  /* Maintain exact-match htab and CIDR fast-path trie */
  {
    op_htab *ht = chan ? chan->bans_ht : global_bans_ht;
    if (ht)
      op_htab_set(ht, p->mask, p, nullptr);
  }
  if (chan)
    ip_trie_add(&chan->ban_ip_trie, p->mask, p);
  if (!noshare) {
    char *mask = str_escape(p->mask, ':', '\\');

    if (mask) {
      if (!chan)
        shareout(nullptr, "+b %s %li %s%s %s %s\n", mask, expire_time - now,
                       ...
                       char *note, time_t expire_time, int flags)
{
  ...
  /* Maintain exact-match htab and CIDR fast-path trie */
  {
    op_htab *ht = chan ? chan->invites_ht : global_invites_ht;
    if (ht)
      op_htab_set(ht, p->mask, p, nullptr);
  }
  if (chan)
    ip_trie_add(&chan->invite_ip_trie, p->mask, p);
  if (!noshare) {
    char *mask = str_escape(p->mask, ':', '\\');

    if (mask) {
      if (!chan)
        shareout(nullptr, "+inv %s %li %s%s %s %s\n", mask, expire_time - now,
                 (flags & MASKREC_STICKY) ? "s" : "",
                 (flags & MASKREC_PERM) ? "p" : "-", from, note);
      else
                       ...
                       char *note, time_t expire_time, int flags)
{
  ...
  /* Maintain exact-match htab and CIDR fast-path trie */
  {
    op_htab *ht = chan ? chan->exempts_ht : global_exempts_ht;
    if (ht)
      op_htab_set(ht, p->mask, p, nullptr);
  }
  if (chan)
    ip_trie_add(&chan->exempt_ip_trie, p->mask, p);
  if (!noshare) {
    char *mask = str_escape(p->mask, ':', '\\');

    if (mask) {
      if (!chan)
        shareout(nullptr, "+e %s %li %s%s %s %s\n", mask, expire_time - now,
                 (flags & MASKREC_STICKY) ? "s" : "",
                 (flags & MASKREC_PERM) ? "p" : "-", from, note);
      else
                        ...
                        struct chanset_t *chan, int show_inact)
{
  ...
  if (ban->added) {
    op_strbuf_appendf(&dates, "%s %s", MODES_CREATED, daysago(now, ban->added));
    if (ban->added < ban->lastactive)
      op_strbuf_appendf(&dates, ", %s %s", MODES_LASTUSED, daysago(now, ban->lastactive));
  }
  if (ban->flags & MASKREC_PERM)
    op_strbuf_appendf(&status, "(perm)%s",
                     (ban->flags & MASKREC_STICKY) ? " (sticky)" : "");
  else
    op_strbuf_appendf(&status, "(expires %s)%s", days(ban->expire, now),
                     (ban->flags & MASKREC_STICKY) ? " (sticky)" : "");
  if (!chan || ischanban(chan, ban->mask)) {
    if (number >= 0)
      dprintf(idx, "  [%3d] %s %s\n", number, ban->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "BAN: %s %s\n", ban->mask, op_strbuf_str(&status));
  } else if (show_inact) {
    if (number >= 0)
      dprintf(idx, "! [%3d] %s %s\n", number, ban->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "BAN (%s): %s %s\n", MODES_INACTIVE, ban->mask, op_strbuf_str(&status));
  } else {
    op_strbuf_free(&dates);
    op_strbuf_free(&status);
    return;
  }
  dprintf(idx, "        %s: %s\n", ban->user, ban->desc);
  if (!op_strbuf_empty(&dates))
    dprintf(idx, "        %s\n", op_strbuf_str(&dates));
  op_strbuf_free(&dates);
  op_strbuf_free(&status);
}
                           ...
                           struct chanset_t *chan, int show_inact)
{
  ...
  op_strbuf_init(&dates);
  if (exempt->added) {
    op_strbuf_appendf(&dates, "%s %s", MODES_CREATED, daysago(now, exempt->added));
    if (exempt->added < exempt->lastactive)
      op_strbuf_appendf(&dates, ", %s %s", MODES_LASTUSED, daysago(now, exempt->lastactive));
  }
  if (exempt->flags & MASKREC_PERM)
    op_strbuf_appendf(&status, "(perm)%s",
                     (exempt->flags & MASKREC_STICKY) ? " (sticky)" : "");
  else
    op_strbuf_appendf(&status, "(expires %s)%s", days(exempt->expire, now),
                     (exempt->flags & MASKREC_STICKY) ? " (sticky)" : "");
  if (!chan || ischanexempt(chan, exempt->mask)) {
    if (number >= 0)
      dprintf(idx, "  [%3d] %s %s\n", number, exempt->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "EXEMPT: %s %s\n", exempt->mask, op_strbuf_str(&status));
  } else if (show_inact) {
    if (number >= 0)
      dprintf(idx, "! [%3d] %s %s\n", number, exempt->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "EXEMPT (%s): %s %s\n", MODES_INACTIVE, exempt->mask, op_strbuf_str(&status));
  } else {
    op_strbuf_free(&dates);
    op_strbuf_free(&status);
    return;
                           ...
                           struct chanset_t *chan, int show_inact)
{
  ...
  op_strbuf_init(&dates);
  if (invite->added) {
    op_strbuf_appendf(&dates, "%s %s", MODES_CREATED, daysago(now, invite->added));
    if (invite->added < invite->lastactive)
      op_strbuf_appendf(&dates, ", %s %s", MODES_LASTUSED, daysago(now, invite->lastactive));
  }
  if (invite->flags & MASKREC_PERM)
    op_strbuf_appendf(&status, "(perm)%s",
                     (invite->flags & MASKREC_STICKY) ? " (sticky)" : "");
  else
    op_strbuf_appendf(&status, "(expires %s)%s", days(invite->expire, now),
                     (invite->flags & MASKREC_STICKY) ? " (sticky)" : "");
  if (!chan || ischaninvite(chan, invite->mask)) {
    if (number >= 0)
      dprintf(idx, "  [%3d] %s %s\n", number, invite->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "INVITE: %s %s\n", invite->mask, op_strbuf_str(&status));
  } else if (show_inact) {
    if (number >= 0)
      dprintf(idx, "! [%3d] %s %s\n", number, invite->mask, op_strbuf_str(&status));
    else
      dprintf(idx, "INVITE (%s): %s %s\n", MODES_INACTIVE, invite->mask, op_strbuf_str(&status));
  } else {
    op_strbuf_free(&dates);
    op_strbuf_free(&status);
    return;
    ...
    if (chan->status & CHAN_ACTIVE) {
      ...
      for (b = chan->channel.ban; b && b->mask[0]; b = b->next) {
        if ((!u_equals_mask(global_bans, global_bans_ht, b->mask)) &&
          ...
          s1 = splitnick(&s2);
          if (s1[0])
            op_strbuf_appendf(&fill, "%s (%s!%s)", b->mask, s1, s2);
          else
            op_strbuf_appendf(&fill, "%s (server %s)", b->mask, s2);
          if (b->timer != 0) {
            min = (now - b->timer) / 60;
            sec = (now - b->timer) - (min * 60);
            op_strbuf_appendf(&fill, " (active %02d:%02d)", min, sec);
          }
          if ((!match[0]) || (match_addr(match, b->mask)))
            dprintf(idx, "* [%3d] %s\n", k, op_strbuf_str(&fill));
          op_strbuf_free(&fill);
          k++;
        }
      }
    ...
    if (chan->status & CHAN_ACTIVE) {
      ...
      for (e = chan->channel.exempt; e && e->mask[0]; e = e->next) {
        if ((!u_equals_mask(global_exempts, global_exempts_ht, e->mask)) &&
          ...
          s1 = splitnick(&s2);
          if (s1[0])
            op_strbuf_appendf(&fill, "%s (%s!%s)", e->mask, s1, s2);
          else
            op_strbuf_appendf(&fill, "%s (server %s)", e->mask, s2);
          if (e->timer != 0) {
            min = (now - e->timer) / 60;
    ...
    if (chan->status & CHAN_ACTIVE) {
      ...
      for (i = chan->channel.invite; i && i->mask[0]; i = i->next) {
        if ((!u_equals_mask(global_invites, global_invites_ht, i->mask)) &&
          ...
          s1 = splitnick(&s2);
          if (s1[0])
            op_strbuf_appendf(&fill, "%s (%s!%s)", i->mask, s1, s2);
          else
            op_strbuf_appendf(&fill, "%s (server %s)", i->mask, s2);
          if (i->timer != 0) {
            min = (now - i->timer) / 60;
...
static int write_bans(FILE *f, int idx)
{
  ...
  for (chan = chanset; chan; chan = chan->next)
    if ((idx < 0) || (chan->status & CHAN_SHARED)) {
      struct flag_record fr = { FR_CHAN | FR_GLOBAL | FR_BOT };
