...
static int tcl_killban STDVAR
{
  ...
  if (u_delban(nullptr, argv[1], 1) > 0) {
...
static int tcl_killexempt STDVAR
{
  ...
  if (u_delexempt(nullptr, argv[1], 1) > 0) {
    for (chan = chanset; chan; chan = chan->next)
...
static int tcl_killinvite STDVAR
{
  ...
  if (u_delinvite(nullptr, argv[1], 1) > 0) {
    for (chan = chanset; chan; chan = chan->next)
...
static int tcl_stick STDVAR
{
  ...
  if (!ok && u_setsticky_ban(nullptr, argv[1], !strncmp(argv[0], "un", 2) ?
      0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", nullptr);
...
static int tcl_stickinvite STDVAR
{
  ...
  if (!ok && u_setsticky_invite(nullptr, argv[1], !strncmp(argv[0], "un", 2) ?
      0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", nullptr);
...
static int tcl_stickexempt STDVAR
{
  ...
  if (!ok && u_setsticky_exempt(nullptr, argv[1], !strncmp(argv[0], "un", 2) ?
      0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", nullptr);
...
static int tcl_channel_info(Tcl_Interp *irp, struct chanset_t *chan)
{
  ...
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_pub_thr, chan->flood_pub_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_ctcp_thr, chan->flood_ctcp_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_join_thr, chan->flood_join_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_kick_thr, chan->flood_kick_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_deop_thr, chan->flood_deop_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->flood_nick_thr, chan->flood_nick_time); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d:%d", chan->aop_min, chan->aop_max); Tcl_AppendElement(irp, op_strbuf_str(&t)); op_strbuf_free(&t); }
  ...
  for (ul = udef; ul; ul = ul->next) {
    ...
    if (ul->type == UDEF_FLAG) {
      ...
      op_strbuf_appendf(&buf, "%s %s", ul->name, p);
...
static int tcl_channel_getlist(Tcl_Interp *irp, struct chanset_t *chan)
{
  ...
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d %d", chan->flood_kick_thr, chan->flood_kick_time); APPEND_KEYVAL("flood-kick", op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d %d", chan->flood_deop_thr, chan->flood_deop_time); APPEND_KEYVAL("flood-deop", op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d %d", chan->flood_nick_thr, chan->flood_nick_time); APPEND_KEYVAL("flood-nick", op_strbuf_str(&t)); op_strbuf_free(&t); }
  { op_strbuf_t t; op_strbuf_appendf(&t, "%d %d", chan->aop_min, chan->aop_max); APPEND_KEYVAL("aop-delay", op_strbuf_str(&t)); op_strbuf_free(&t); }
                           ...
                           char *setting)
{
  ...
  if (!strcmp(setting, "chanmode"))
    ...
    Tcl_AppendResult(irp, chan->need_op, nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "need-invite")) {
    Tcl_AppendResult(irp, chan->need_invite, nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "need-key")) {
    Tcl_AppendResult(irp, chan->need_key, nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "need-unban")) {
    Tcl_AppendResult(irp, chan->need_unban, nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "need-limit")) {
    Tcl_AppendResult(irp, chan->need_limit, nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "idle-kick")) {
    Tcl_AppendResult(irp, int_to_base10(chan->idle_kick), nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "stopnethack-mode") || !strcmp(setting, "stop-net-hack")) {
    Tcl_AppendResult(irp, int_to_base10(chan->stopnethack_mode), nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "revenge-mode")) {
    Tcl_AppendResult(irp, int_to_base10(chan->revenge_mode), nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "ban-type")) {
    Tcl_AppendResult(irp, int_to_base10(chan->ban_type), nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "ban-time")) {
    Tcl_AppendResult(irp, int_to_base10(chan->ban_time), nullptr);
    return TCL_OK;
  } else if (!strcmp(setting, "exempt-time")) {
    Tcl_AppendResult(irp, int_to_base10(chan->exempt_time), nullptr);
    return TCL_OK;
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_pub_thr, chan->flood_pub_time);
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_ctcp_thr, chan->flood_ctcp_time);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_join_thr, chan->flood_join_time);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_kick_thr, chan->flood_kick_time);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_deop_thr, chan->flood_deop_time);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->flood_nick_thr, chan->flood_nick_time);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    op_strbuf_t t;
    op_strbuf_init(&t);
    op_strbuf_appendf(&t, "%d %d", chan->aop_min, chan->aop_max);
    Tcl_AppendResult(irp, op_strbuf_str(&t), nullptr);
    op_strbuf_free(&t);
    return TCL_OK;
  }
    ...
    if (ul->type == UDEF_STR) {
      ...
      Tcl_SplitList(irp, str, &argc, &argv);
      if (argc > 0)
        Tcl_AppendResult(irp, argv[0], nullptr);
      Tcl_Free((char *) argv);
    } else {
...
static int tcl_channel STDVAR
{
    ...
    if (chan == nullptr) {
      if (chan_hack == 1)
        return TCL_OK;          /* Ignore channel settings for a static
                                 ...
                                 * the config */
      Tcl_AppendResult(irp, "no such channel record", nullptr);
      return TCL_ERROR;
    }
  ...
  Tcl_AppendResult(irp, "unknown channel command: should be one of: ",
                   "add, set, get, info, remove", nullptr);
  return TCL_ERROR;
}
...
static int tcl_do_masklist(maskrec *m, Tcl_Interp *irp)
{
  ...
  for (; m; m = m->next) {
    ...
    op_strbuf_appendf(&ts, "%" PRId64, (int64_t) m->expire);
    op_strbuf_appendf(&ts1, "%" PRId64, (int64_t) m->added);
    op_strbuf_appendf(&ts2, "%" PRId64, (int64_t) m->lastactive);
    list[0] = m->mask;
    list[1] = m->desc;
...
static int tcl_savechannels STDVAR
{
  ...
  if (!chanfile[0]) {
    Tcl_AppendResult(irp, "no channel file", nullptr);
    return TCL_ERROR;
  }
  write_channels();
  return TCL_OK;
...
static int tcl_setlaston STDVAR
{
  ...
  u = get_user_by_handle(userlist, argv[1]);
  if (!u) {
    Tcl_AppendResult(irp, "No such user: ", argv[1], nullptr);
    return TCL_ERROR;
  }
  if (argc == 4)
    t = (time_t) egg_atoi(argv[3]);
  if (argc == 3 && ((argv[2][0] != '#') && (argv[2][0] != '&')))
    t = (time_t) egg_atoi(argv[2]);
...
static int tcl_addchanrec STDVAR
{
  ...
  if (get_chanrec(u, argv[2]) != nullptr) {
    Tcl_AppendResult(irp, "0", nullptr);
    return TCL_OK;
  }
...
static int tcl_chansettype STDVAR
{
  ...
  if (!strcmp(argv[1], "chanmode") ||
    ...
    Tcl_AppendResult(irp, "str", nullptr);
  /* Couplets */
    ...
    Tcl_AppendResult(irp, "pair", nullptr);
  /* Integers now */
  ...
  /* Last, but not least - flags */
    ...
    if (ul->type == UDEF_STR)
      ...
      Tcl_AppendResult(irp, "flag", nullptr);
         ...
         */
      Tcl_AppendResult(irp, "unknown", nullptr);
  }
