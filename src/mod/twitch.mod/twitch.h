...
typedef struct userstate {
  ...
  int badge_info;
  char *badges;           /* dynamically allocated; nullptr until first USERSTATE */
  char color[8];
...
typedef struct twitchchan {
  ...
  char *mods;   /* dynamically allocated; nullptr until /mods response */
  char *vips;   /* dynamically allocated; nullptr until /vips response */
