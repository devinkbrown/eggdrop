...
static int msg_addhost(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!par[0]) {
    ...
    if (u_pass_match(u, "-")) {
      ...
      check_this_user(u->handle, 0, nullptr);
      return 1;
    }
  }
...
static int msg_info(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (par[0]) {
    ...
    if (p && (p[0] == '@'))
      locked = 1;
    if (chname) {
      get_handle_chaninfo(u->handle, chname, s, sizeof s);
      if (s[0] == '@')
        locked = 1;
    }
    if (locked) {
...
static int msg_who(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  for (m = chan->channel.member; m && m->nick[0]; m = m->next) {
    ...
    if (info && (info[0] == '@'))
      ...
      get_handle_chaninfo(mu->handle, chan->dname, s, sizeof s);
      if (s[0]) {
        info = s;
        if (info[0] == '@')
          info++;
    ...
    if (info && info[0])
      ...
      if (match_my_nick(m->nick))
        ...
        i = (int)(stpcpy(also + i, m->nick) - also);
        if (i > 400) {
...
static int msg_whois(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  for (chan = chanset; chan; chan = chan->next) {
    if (hand_on_chan(chan, u2)) {
      op_strbuf_clear(&s1);
      op_strbuf_appendf(&s1, "NOTICE %s :[%s] %s: %s.", nick, u2->handle,
      ...
      if (cr && (cr->laston > tt) && (!channel_hidden(chan) ||
        ...
        ok = 1;
        op_strbuf_clear(&s1);
        op_strbuf_appendf(&s1, "NOTICE %s :[%s] %s %s on %s", nick,
                          u2->handle, IRC_LASTSEENAT, stime, chan->dname);
      }
    }
  }
  if (!ok)
    op_strbuf_appendf(&s1, "NOTICE %s :[%s] %s", nick, u2->handle,
                      IRC_NEVERJOINED);
  dprintf(DP_HELP, "%s\n", op_strbuf_str(&s1));
  op_strbuf_free(&s1);
...
static int msg_help(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (helpdir[0]) {
    struct flag_record fr = { FR_ANYWH | FR_GLOBAL | FR_CHAN };
...
static int msg_status(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  dprintf(DP_HELP, "NOTICE %s :I am %s, running %s: %d user%s  (rss: %ldk).\n",
          nick, botnetnick, ver, i, i == 1 ? "" : "s", egg_get_rss_kb());

  {
    char upbuf[64];
    egg_format_uptime(now - online_since, upbuf, sizeof upbuf);
    dprintf(DP_HELP, "NOTICE %s :%s %s.\n", nick, MISC_ONLINEFOR, upbuf);
  }
...

  {
    ...
    for (chan = chanset; chan; chan = chan->next) {
      ...
      if (!channel_active(chan))
        ...
        op_strbuf_append_cstr(&_s, " (pending)");
      else if (!me_op(chan))
        op_strbuf_append_cstr(&_s, " (need ops)");
      op_strbuf_append_cstr(&_s, ", ");
 ...
 */
static cmd_t C_msg[] = {
  {"addhost", "",    (IntFunc) msg_addhost, nullptr},
  {"die",     "n",   (IntFunc) msg_die,     nullptr},
  {"go",      "",    (IntFunc) msg_go,      nullptr},
  {"hello",   "",    (IntFunc) msg_hello,   nullptr},
  {"help",    "",    (IntFunc) msg_help,    nullptr},
  {"ident",   "",    (IntFunc) msg_ident,   nullptr},
  {"info",    "",    (IntFunc) msg_info,    nullptr},
  {"invite",  "o|o", (IntFunc) msg_invite,  nullptr},
  {"jump",    "m",   (IntFunc) msg_jump,    nullptr},
  {"key",     "o|o", (IntFunc) msg_key,     nullptr},
  {"memory",  "m",   (IntFunc) msg_memory,  nullptr},
  {"op",      "",    (IntFunc) msg_op,      nullptr},
  {"halfop",  "",    (IntFunc) msg_halfop,  nullptr},
  {"pass",    "",    (IntFunc) msg_pass,    nullptr},
  {"password","",    (IntFunc) msg_pass,    nullptr},
  {"rehash",  "m",   (IntFunc) msg_rehash,  nullptr},
  {"reset",   "m",   (IntFunc) msg_reset,   nullptr},
  {"save",    "m",   (IntFunc) msg_save,    nullptr},
  {"status",  "m|m", (IntFunc) msg_status,  nullptr},
  {"voice",   "",    (IntFunc) msg_voice,   nullptr},
  {"who",     "",    (IntFunc) msg_who,     nullptr},
  {"whois",   "",    (IntFunc) msg_whois,   nullptr},
