...
constexpr int BAN    = 0x08;
constexpr int VOICE  = 0x10;
constexpr int EXEMPT = 0x20;
constexpr int INVITE = 0x40;
                    ...
                    char *ch, struct userrec *u)
{
  ...
  if (!match_my_nick(nick)) {
    ...
    for (m = chan->channel.member; m && m->nick[0]; m = m->next) {
      op_strbuf_clear(&_s1);
      op_strbuf_appendf(&_s1, "%s!%s", m->nick, m->userhost);
      ...
      for (b = cycle ? chan->bans : global_bans; b; b = b->next) {
        if (match_addr(b->mask, who)) {
          if (b->desc && b->desc[0] != '@') {
            ...
            op_strbuf_appendf(&_resn, "%s %s", IRC_PREBANNED, b->desc);
          } else
            op_strbuf_clear(&_resn);
        }
      }
