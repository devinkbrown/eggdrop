...
struct dcc_t *dcc = nullptr;       /* DCC list                                   */
...
static void escape_telnet(op_strbuf_t *out, const char *s)
{
  op_strbuf_clear(out);
  for (; *s; s++) {
    if ((unsigned char) *s == TLN_IAC)
      op_strbuf_appendc(out, *s);
    else if (*s == '\n')
      op_strbuf_appendc(out, '\r');
    op_strbuf_appendc(out, *s);
  }
}
...
static void bot_version(int idx, char *par)
{
  ...
  if ((par[0] >= '0') && (par[0] <= '9')) {
    ...
    work = newsplit(&par);
    dcc[idx].u.bot->numver = egg_atoi(work);
  } else
    dcc[idx].u.bot->numver = 0;

  dprintf(idx, "tb %s\n", botnetnick);
  l = egg_atoi(newsplit(&par));
  if (l != HANDLEN) {
  ...
  op_strlcpy(dcc[idx].u.bot->version, par, sizeof dcc[idx].u.bot->version);
...
void failed_link(int idx)
{
  ...
  if (dcc[idx].u.bot->linker[0]) {
    ...
    op_strlcpy(s1, dcc[idx].u.bot->linker, sizeof(s1));
  ...
  killsock(dcc[idx].sock);
  op_strlcpy(nick, dcc[idx].nick, sizeof nick);
  lostdcc(idx);
    ...
    if (in_chain(dcc[idx].nick)) {
      ...
      if (i > 0) {
        ...
        op_strbuf_appendf(&x,
                     ...
                     users, (users != 1) ? "s" : "");
        putlog(LOG_BOTS, "*", "%s.", op_strbuf_str(&x));
        botnet_send_unlinked(i, dcc[i].nick, op_strbuf_str(&x));
        op_strbuf_free(&x);
        dprintf(i, "bye %s\n", "restructure");
        killsock(dcc[i].sock);
        lostdcc(i);
...
static void dcc_bot_new(int idx, char *buf, int x)
{
  ...
  code = newsplit(&buf);
  if (!op_strcasecmp(code, "*hello!"))
    greet_new_bot(idx);
  else if (!op_strcasecmp(code, "version") || !op_strcasecmp(code, "v"))
    bot_version(idx, buf);
  else if (!op_strcasecmp(code, "badpass"))
    /* We entered the wrong password */
    putlog(LOG_BOTS, "*", DCC_BADPASS, dcc[idx].nick);
  else if (!op_strcasecmp(code, "passreq")) {
    pass = get_bot_pass(u);
    if (!pass || !strcmp(pass, "-")) {
  ...
  } else if (!op_strcasecmp(code, "error"))
...
static void eof_dcc_bot(int idx)
{
  ...
  op_strbuf_appendf(&x,
               ...
               (users != 1) ? "s" : "");
  putlog(LOG_BOTS, "*", "%s.", op_strbuf_str(&x));
  botnet_send_unlinked(idx, dcc[idx].nick, op_strbuf_str(&x));
  op_strbuf_free(&x);
  killsock(dcc[idx].sock);
  lostdcc(idx);
}
...
static void dcc_chat_pass(int idx, char *buf, int atr)
{
  ...
  if ((atr & USER_BOT) && !op_strncasecmp(buf, "digest ", 7)) {
  ...
  if (u_pass_match(dcc[idx].user, buf)) {
    ...
    if (dcc[idx].u.chat->away) {        /* su from a dumb user */
      ...
      op_strlcpy(dcc[idx].nick, dcc[idx].u.chat->away, sizeof dcc[idx].nick);
      op_free(dcc[idx].u.chat->away);
      op_free(dcc[idx].u.chat->su_nick);
      dcc[idx].u.chat->away = nullptr;
      dcc[idx].u.chat->su_nick = nullptr;
      dcc[idx].type = &DCC_CHAT;
  ...
  while (*text) {
    ...
    case 3:                    /* mIRC colors? */
      if (flags & STRIP_COLOR) {
        if (isdigit((unsigned char)(text[1]))) { /* Is the first char a number? */
...
static void append_line(int idx, char *line)
{
  ...
  if ((c->line_count < c->max_line) && (c->buffer == nullptr)) {
    c->line_count++;
...
static void out_dcc_general(int idx, char *buf, void *x)
{
  ...
  if (dcc[idx].status & STAT_TELNET) {
    ...
    if (dcc[idx].status & STAT_PAGE)
      append_line(idx, (char *) op_strbuf_str(&escaped));
    else
      tputs(dcc[idx].sock, op_strbuf_str(&escaped), op_strbuf_len(&escaped));
    op_strbuf_free(&escaped);
  } else {
    if (dcc[idx].status & STAT_PAGE)
      append_line(idx, buf);
    else
...
static void dcc_chat(int idx, char *buf, int i)
{
  ...
  if (buf[0]) {
    ...
    if (filt != buf) {
      op_strlcpy(filtbuf, filt, sizeof(filtbuf));
      buf = filtbuf;
    }
  }
  if (buf[0]) {
      ...
      if ((buf[0] == '.') || (dcc[idx].u.chat->channel < 0)) {
        ...
        if (check_tcl_dcc(v, idx, buf)) {
          ...
          if (dcc[idx].u.chat->su_nick) {
            ...
            op_strlcpy(dcc[idx].nick, dcc[idx].u.chat->su_nick, sizeof dcc[idx].nick);
            dcc[idx].type = &DCC_CHAT;
        ...
        if ((buf[1] == 'm') && (buf[2] == 'e') && buf[3] == ' ')
          me = 1;
        for (i = 0; i < dcc_total; i++) {
          bool ok = (dcc[i].type->flags & DCT_MASTER) &&
...
static int detect_telnet_flood(const char *floodhost)
{
  ...
  get_user_flagrec(get_user_by_host(floodhost), &fr, nullptr);
  if (!flood_telnet_thr || (glob_friend(fr) && !par_telnet_flood))
    return 0;                   /* No flood protection */
  if (op_strcasecmp(lasttelnethost, floodhost)) {      /* New... */
    op_strlcpy(lasttelnethost, floodhost, sizeof lasttelnethost);
    lasttelnettime = now;
    lasttelnets = 0;
    return 0;
  }
...
static void dcc_telnet(int idx, char *buf, int i)
{
...
#endif
  dcc[i].timeval = now;
  op_strlcpy(dcc[i].nick, "*", HANDLEN + 1);
  ...
  if (!strcmp(dcc[idx].nick, "(webui)")) {
    ...
    op_strlcpy(dcc[i].u.dns->host, ip, len);
...
void dcc_telnet_hostresolved2(int i, int idx) {
  ...
  op_strlcpy(dcc[j].host, dcc[i].host, UHOSTLEN);
  op_strlcpy(dcc[j].nick, "*", HANDLEN + 1);
...
static void dcc_telnet_hostresolved(int i)
{
  ...
  op_strlcpy(dcc[i].host, dcc[i].u.dns->host, UHOSTLEN);
  ...
  if ((dcc[idx].status & LSTN_PUBLIC) && !strcmp(dcc[idx].nick, "(script)")) {
    ...
    op_strlcpy(dcc[i].nick, "*", HANDLEN + 1);
    op_strlcpy(dcc[i].host, userhost, UHOSTLEN);
    op_strbuf_free(&s);
...
static void dcc_telnet_id(int idx, char *buf, int atr)
{
  ...
  if (!op_strcasecmp(buf, "NEW") && (allow_new_telnets || make_userfile)) {
  ...
  correct_handle(buf);
  op_strlcpy(dcc[idx].nick, buf, sizeof dcc[idx].nick);
  if (glob_bot(fr)) {
    if (!op_strcasecmp(botnetnick, dcc[idx].nick)) {
      /* change here temp to use bot output */
      dcc[idx].type = &DCC_BOT_NEW;
...
static void dcc_telnet_new(int idx, char *buf, int x)
{
  ...
  if (!ok) {
  ...
  } else if (strchr(BADHANDCHARS, buf[0]) != nullptr) {
    ...
    dprintf(idx, "Try another one please:\n");
    return;
  } else if (!op_strcasecmp(buf, botnetnick))
    dprintf(idx, "Sorry, can't use my name for a nick.\n");
  else {
    op_strlcpy(dcc[idx].nick, buf, HANDLEN + 1);
    if (make_userfile)
...
static void dcc_telnet_pw(int idx, char *new, int x)
{
  ...
  if (notify_new[0]) {
    ...
    op_strlcpy(notify_list, notify_new, sizeof(notify_list));
...
void dcc_ident(int idx, char *buf, int len)
{
  ...
  *response = *uid = '\0';
  ...
  rmspace(uid);
  uid[sizeof uid - 1] = '\0';
  if (op_strncasecmp(response, "USERID", 6) || strchr(uid, '@')) {
...
static void dcc_telnet_got_ident(int i, const char *host)
{
  ...
  op_strlcpy(dcc[i].host, host, UHOSTLEN);
  op_strbuf_t x = {};
  op_strbuf_init(&x);
  ...
  if (protect_telnet && !make_userfile) {
    ...
    if (ok && require_p && !(u->flags & USER_PARTY))
      ok = false;
    else if (ok && !require_p && !(u->flags & USER_OP))
      ok = false;
  ...
  if (match_ignore(op_strbuf_str(&x))) {
    op_strbuf_free(&x);
    killsock(dcc[i].sock);
    lostdcc(i);
    return;
  ...
  if (!strcmp(dcc[idx].nick, "(script)")) {
    ...
    dcc[i].u.other = nullptr;
    op_strlcpy(dcc[i].nick, "*", sizeof(dcc[i].nick));
    check_tcl_listen(dcc[idx].host, dcc[i].sock);
    return;
  }
  ...
  op_memzero(dcc[i].u.chat, sizeof(struct chat_info));
  ...
  /* Copy acceptable-nick/host mask */
  op_strlcpy(dcc[i].nick, dcc[idx].host, sizeof dcc[i].nick);
