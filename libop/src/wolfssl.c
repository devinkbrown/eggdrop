...
op_ssl_connect_common(op_fde_t *const F, void *const data)
{
	...
	{
		{
			/* Log the negotiated ALPN protocol so we can verify "dot"
			 * was agreed for outgoing DoT connections. */
			...
			op_lib_log("TLS connect: negotiated %s (cipher: %s, ALPN: %.*s)",
			           ...
			           alpn_proto_len ? (int)alpn_proto_len : 4,
			           alpn_proto_len ? alpn_proto : "none");
		}
