...
op_init_netio_uring(void)
{
	...
	return uring_init_into(&legacy_uring_data);
}
...
op_select_uring(long delay)
{
	...
	if (__builtin_expect(d->thread_active, 0))
	{
		...
		uint64_t notify_val;
		ssize_t r;
		do { r = read(d->notify_fd, &notify_val, sizeof notify_val); }
...
uring_cq_poll_thread_fn(void *arg)
{
	...
	while (!d->thread_stop)
	{
		...
		/*
		 * Fast path: if a CQE is already available, skip the submit
		 * syscall entirely — io_uring_peek_cqe is a userspace memory
		 * read.  This is the common case under load when the kernel
		 * has produced CQEs in batch and no new SQEs need flushing.
		 */
		if (io_uring_peek_cqe(&d->uring, &cqe) == 0 && cqe != NULL)
		{
			/* CQEs available — no need to submit/wait, fall through to drain. */
		}
		else
		{
			/*
			 * No CQEs immediately available.  Combine submit+wait
			 * into a single io_uring_enter() syscall using
			 * io_uring_submit_and_wait_timeout when the SQ has
			 * pending entries.  Otherwise plain wait suffices.
			 *
			 * sqlock is held briefly to serialise SQ access with
			 * the I/O thread's uring_flush_dirty.  The wait itself
			 * happens outside the lock so we don't extend the
			 * window holding it (mission constraint).
			 */
			unsigned int to_submit;

			pthread_spin_lock(&d->sqlock);
			to_submit = io_uring_sq_ready(&d->uring);
			if (to_submit > 0)
			{
				/* Combined submit + wait in one syscall. */
				ret = io_uring_submit_and_wait_timeout(
					&d->uring, &cqe, 1, &ts, NULL);
				pthread_spin_unlock(&d->sqlock);
			}
			else
			{
