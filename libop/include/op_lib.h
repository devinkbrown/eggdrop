/*
 * libop/include/op_lib.h — master umbrella header for eggdrop's libop.
 *
 * Covers: portable data structures, utilities, and the commio I/O layer.
 * TLS is provided by wolfSSL (wolfssl.c) or no-op stubs (wolfssl_nossl.c).
 *
 * Copyright (C) 1990 Jarkko Oikarinen and University of Oulu, Co Center
 * Copyright (C) 1996-2002 Hybrid Development Team
 * Copyright (C) 2002-2005 ircd-ratbox development team
 * Copyright (C) 2024-2026 Ophion IRC Daemon contributors / eggdrop contributors
 * GPL-2.0-or-later
 */
...
# define slop_assert(expr) \
    (op_likely(expr) || \
     (op_lib_log("assert failed: %s (%s:%d)", #expr, __FILE__, __LINE__), 0))
#endif
...
/* -------------------------------------------------------------------------
 * Size constants (used by commio.c inet helpers)
 * ---------------------------------------------------------------------- */
#ifndef HOSTIPLEN
# define HOSTIPLEN   53
#endif
 ...
 * from meson-injected macros.  Retrieve via op_build_info().
 */
typedef struct op_build_info {
	const char *product;      /* e.g. "eggdrop" or "ophion"             */
	const char *version;      /* meson project_version(), e.g. "1.9.6" */
...
/* -------------------------------------------------------------------------
 * Portable data-structure and utility headers
 * ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * commio I/O layer headers
 * op_commio.h must precede op_rawbuf.h (rawbuf uses op_fde_t from commio)
 * ---------------------------------------------------------------------- */
