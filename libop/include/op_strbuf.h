...
/*
 * op_strbuf_printf — libop replacement for snprintf.
 *
 * Initialises a fresh strbuf, formats into it, and never truncates.
 * The output grows on demand; no buffer-size arithmetic required.
 *
 * Replaces:
 *   char buf[NICKLEN + UHOSTLEN];           // guess at the size
 *   snprintf(buf, sizeof buf, "%s!%s", nick, host);   // silently truncates
 *   use(buf);
 *
 * With:
 *   op_strbuf_t sb;
 *   op_strbuf_printf(&sb, "%s!%s", nick, host);       // always exact
 *   use(op_strbuf_str(&sb));
 *   op_strbuf_free(&sb);
 *
 * The caller owns the strbuf and must call op_strbuf_free() when done.
 * Returns 0 on success (OOM is fatal in libop and never reached in practice).
 */
static inline int op_strbuf_printf(op_strbuf_t *sb, const char *fmt, ...) AFP(2, 3);
static inline int
op_strbuf_printf(op_strbuf_t *sb, const char *fmt, ...)
{
	va_list ap;
	int r;
	op_strbuf_init(sb);
	va_start(ap, fmt);
	r = op_strbuf_vappendf(sb, fmt, ap);
	va_end(ap);
	return r;
}

/*
 * op_strbuf_vprintf — va_list form of op_strbuf_printf.
 *
 * Useful when wrapping op_strbuf_printf in your own vararg function:
 *
 *   void my_format(op_strbuf_t *sb, const char *fmt, ...)
 *   {
 *       va_list ap;
 *       va_start(ap, fmt);
 *       op_strbuf_vprintf(sb, fmt, ap);
 *       va_end(ap);
 *   }
 */
static inline int
op_strbuf_vprintf(op_strbuf_t *sb, const char *fmt, va_list ap)
{
	op_strbuf_init(sb);
	return op_strbuf_vappendf(sb, fmt, ap);
}

/*
 * op_strbuf_reset — clear an existing strbuf and re-format it.
 *
 * Like op_strbuf_printf but reuses the strbuf's already-allocated heap
 * memory instead of re-initialising.  Preferable inside loops where the
 * same strbuf is reformatted many times — avoids repeated malloc/free:
 *
 *   op_strbuf_t sb;
 *   op_strbuf_init(&sb);
 *   for (...) {
 *       op_strbuf_reset(&sb, "NOTICE %s :%s", nick, chan->dname);
 *       send(op_strbuf_str(&sb));
 *   }
 *   op_strbuf_free(&sb);
 */
static inline int op_strbuf_reset(op_strbuf_t *sb, const char *fmt, ...) AFP(2, 3);
static inline int
op_strbuf_reset(op_strbuf_t *sb, const char *fmt, ...)
{
	va_list ap;
	int r;
	op_strbuf_clear(sb);
